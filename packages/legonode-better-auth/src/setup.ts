...
import type { LegonodePluginSetupFn } from "legonode";
...
const AUTH_PLUGIN_TEMPLATE = `import { BetterAuthPlugin } from "@legonode/better-auth";
...
const AUTH_ROUTE_TEMPLATE = `import { createBetterAuthRouteHandler } from "@legonode/better-auth";
...
export const setup: LegonodePluginSetupFn = async (ctx) => {
  ...
  const result = await ctx.addPluginToLegonodeConfig({
  ...
  if (!result.ok) {
    ctx.log.warn(`Could not auto-edit legonode.config.ts: ${result.error}`);
