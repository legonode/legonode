...
export const BUILTIN_TEMPLATES: Record<string, string> = {
  basic: "https://github.com/legonode/legonode/tree/dev/templates/basic",
};
...
async function resolveTemplateChoice(
  ...
  const choices = [
    ...builtinChoices,
    {
      ...
      description: "Minimal Legonode app scaffold in this CLI",
    },
  ];
...
function cloneGithubTreeFolder(spec: ParsedGithubTree, targetDir: string): void {
  ...
  const tmpRoot = mkdtempSync(join(tmpdir(), "legonode-create-"));
