...
import { addPluginToLegonodeConfig } from "../../src/setup/addPluginToLegonodeConfig.js";
import type { LegonodePluginSetupFn } from "../../src/setup/pluginSetupTypes.js";
...
export const addCommand = {
  async run(opts: AddCommandOptions): Promise<void> {
    ...
    if (!packageName?.trim()) {
      appLogger.error("Missing package name. Usage: legonode add -p <package>");
      process.exit(1);
    }
    ...
    let setupFn: LegonodePluginSetupFn | undefined;
    try {
      const mod = await import(/* webpackIgnore: true */ spec);
      setupFn = (mod.setup ?? mod.default) as LegonodePluginSetupFn | undefined;
    } catch (e: unknown) {
      ...
      if (
        ...
        appLogger.info(
          "No setup script found for this package (missing exported `./setup` or `setup` function). Add the plugin to legonode.config.ts manually.",
        );
        return;
      }
    ...
    if (typeof setupFn !== "function") {
      appLogger.info(
        "No setup function found for this package (expected named export `setup` or default export). Add the plugin to legonode.config.ts manually.",
      );
      return;
    }
    ...
    const ctx = {
      ...
      addPluginToLegonodeConfig: (input: Parameters<typeof addPluginToLegonodeConfig>[1]) =>
        addPluginToLegonodeConfig(cwd, input),
      log: {
