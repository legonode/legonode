...
/** Import line to merge into `legonode.config.ts`. */
export type LegonodeConfigImportSpec =
  | { kind: "named"; names: string[]; module: string }
  | { kind: "default"; name: string; module: string };

export type AddPluginToLegonodeConfigInput = {
  /** Imports to add or merge (same `module` updates one import declaration). */
  imports?: LegonodeConfigImportSpec[];
  ...
  /** Relative to `cwd`. Default: `legonode.config.ts`. */
  configFile?: string;
};

export type AddPluginToLegonodeConfigResult =
...
function parseExpression(expr: string, languageVersion: ts.ScriptTarget): ts.Expression {
  const wrapped = `const __legonode_plugin_expr = ${expr};`;
...
function mergeImports(
  sf: ts.SourceFile,
  specs: LegonodeConfigImportSpec[],
...
function applyPluginExpressions(
  ...
  if (!pluginsProp) {
    return {
      ok: false,
      error:
        'Could not find `plugins` array on config. Add `plugins: []` to legonode.config.ts first.',
    };
  }
...
/**
 * Updates `legonode.config.ts`: merges imports and appends plugin call expressions to `plugins`.
 * Intended for use from plugin setup scripts (see {@link LegonodePluginSetupContext}).
 */
export async function addPluginToLegonodeConfig(
  cwd: string,
  input: AddPluginToLegonodeConfigInput,
): Promise<AddPluginToLegonodeConfigResult> {
  const fileName = input.configFile ?? "legonode.config.ts";
