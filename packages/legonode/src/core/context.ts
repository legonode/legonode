...
export type LegonodeResponseTyped<T extends ResponseBodyByStatus> = Omit<
  LegonodeResponse,
  "status" | "json"
> & {
...
function createLegonodeResponse(
  nodeRes: ServerResponse,
  defaultStatusRef: DefaultStatusRef,
...
function allocContext(
  ...
  const ctxRef: { current: LegonodeContext | null } = { current: null };
...
export function releaseContext(ctx: LegonodeContext): void {
