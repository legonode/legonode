 ...
 */
export async function getMethodsExportedByRouteFile(filePath) {
    try {
        const url = pathToFileURL(filePath).href;
        const bust = process.env.LEGONODE_RELOAD_TOKEN;
        if (bust)
...
export async function loadRoute(filePath, method, routeId) {
    try {
        const url = pathToFileURL(filePath).href;
        const bust = process.env.LEGONODE_RELOAD_TOKEN;
        if (bust)
        ...
        return {
            routeId,
            handler: handlerFn,
            ...(mod.SCHEMA != null && { schema: mod.SCHEMA }),
            ...(mod[`${method}_SCHEMA`] != null && {
                methodSchema: mod[`${method}_SCHEMA`],
            }),
...
export async function reloadRoutes({ currentDir, changedPath, fileExists, filename, }) {
    const table = getRouteTable(currentDir);
    const routeEntries = table.filter((r) => resolve(r.filePath) === resolve(changedPath));
    ...
    if (!fileExists && routeEntries.length > 0) {
        ...
        appLogger.info(`[legonode] delete route: ${label} (${filename})`);
        return;
    }
    else if (routeEntries.length > 0) {
        const token = String(Date.now());
        process.env.LEGONODE_RELOAD_TOKEN = token;
        await reloadRoute(currentDir, changedPath);
        appLogger.info(`[legonode] reload: ${label} (${filename})`);
        return;
    }
    addRouteFileToTable(currentDir, changedPath);
    appLogger.info(`[legonode] add route: ${label} (${filename})`);
    return;
}
