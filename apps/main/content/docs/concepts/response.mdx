...
If you define `RESPONSE_SCHEMA` (or `GET_RESPONSE_SCHEMA`, etc.) in your route file, Legonode can type `ctx.res.status(code).json(data)` and use fast serialization for that shape.
