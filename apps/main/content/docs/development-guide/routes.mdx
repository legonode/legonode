...
- **Serialization** — When a response schema is available for the status code, Legonode can use fast serialization for that shape.
...
- `GET_RESPONSE_SCHEMA` tells Legonode the shape of the 200 response. With `Context<Res>`, TypeScript checks that the object you pass to `.json()` matches that shape.
