...
icon: CalendarClock
title: Cron Plugin
description: Use @legonode/cron to run scheduled tasks from appDir/cron and trigger jobs with ctx.schedule().
---

# Cron Plugin

Cron is provided by the **`@legonode/cron`** plugin.

Install and register it in `legonode.config.ts`:
